...
public class P1779FindNearestPointThatHasTheSameXOrYCoordinate{
    
    @Test
    public void testResult() {
        Solution solution = new P1779FindNearestPointThatHasTheSameXOrYCoordinate().new Solution();        // TO TEST
        int x = 3, y = 4;
...
class Solution {
    public int nearestValidPoint(int x, int y, int[][] points) {
        ...
        for (int i = 0; i < points.length; i++) {
            ...
            int yi = points[i][1];
            if(xi == x || yi == y) {
                int man = Math.abs(xi - x) + Math.abs(yi - y);
                if (man < dist) {
                    dist = man;
                    idx = i;
                }
